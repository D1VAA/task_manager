...
@contextmanager
def get_db():
    """Contextmanager para gerenciar a sessão com o banco de dados. Garantindo que a sessão seja aberta e fechada corretamente."""
    db = LocalSession()
    try:
        yield db
    finally:
...
def create_freight(origem: str, destino: str, client: str, link: str):
    ...
    Args:
        origem (str): A origem da prestação do serviço. O ideal é que a UF seja informada junto. Ex: 'Cidade, UF'
        destino (str): O destino do frete. O ideal é que a UF seja informada junto com a cidade. Ex: 'Cidade, UF'
    ...
    with get_db() as db:
        ...
        try:
            ...
            db.rollback()
            return {'message': f'An error occurred while creating the freight.', 'error': e}

def query_freight(origem: Optional[str] = None, 
                  destino: Optional[str] = None, 
                  client: Optional[str] = None) -> List[FreightOBJ]:
    ...
    with get_db() as db:
        ...
        freights = query.all()
        return [FreightOBJ(freight.origem, 
                            freight.destino, 
                            freight.client, 
                            freight.link) for freight in freights]

def delete_freight(freight_id: int) -> Union[Dict[str, str], Dict[str, Union[ Exception, str]]]:
    """
    ...
    with get_db() as db:
        ...
        if freight is not None:
            ...
            return {"error": f"Freight id not informed."}
