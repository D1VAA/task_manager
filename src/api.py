...
from utils.imports import query_freight, Optional, Dict
...
async def create_freight(filename, file_content: bytes):
    ...
    try:
        if file_content:
            ...
            return {'message': 'File upload succesfully.', 'file_link': file_link}
        else:
...
async def api_upload_freight(origem: str = Form(...), 
                            destino: str = Form(...),
                            cliente: str = Form(...),
    ...
    try:
        ...
        print(e)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Error uploading the freight: {str(e)}')
